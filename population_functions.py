...
def GetNextProportions(CurrentDistance_, CurrentProportions_):
    '''Return the population proportions for each individual GRN instances based on their fitness'''
...
def GetLongJumpList(GRNlist, Proportionlist_, SelectionPower_):
    '''Return the indices of instances that are going to have a long jump (The last SelectionPower%)'''
    length = math.ceil(len(GRNlist)//(1/SelectionPower_))
    LongJumpIndexList = []
    for i in range(0, length):
...
def InitiateGlobalMutationDirection(totalnumberofgenes):
    '''Helper function for initialization of mutation parameters (adjacency matrix)'''
...
def InitiateGlobalMutationDirection_LG(totalnumberofgenes):
    '''Helper function for initialization of mutation parameters (logic gates)'''
